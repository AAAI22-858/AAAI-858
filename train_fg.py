...
def main():
    ...
    for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(dataset, start=epoch_iter):
            ...
            loss_G_fg = loss_dict['G_GAN'] + loss_dict.get('G_GAN_Feat', 0) + loss_dict.get('G_VGG', 0) + loss_dict['Dt_fake'] * 0.25
        ...
        if epoch % opt.save_epoch_freq == 0:
            ...
            torch.save(optimizer_D.state_dict(), os.path.join(checkpoint_folder, f"{epoch}_optimizer_D.pth"))
            # torch.save(generator.state_dict(), os.path.join(self.ckpt_dir, '%05d_generator.pth' % batch_num))
            torch.save(optimizer_Dt.state_dict(), os.path.join(checkpoint_folder, f"{epoch}_optimizer_Dt.pth"))
            torch.save(optimizer_G.state_dict(), os.path.join(checkpoint_folder, f"{epoch}_optimizer_G.pth"))
